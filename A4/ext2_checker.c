 ...
 * Output of a.
 */
unsigned void fix_output(char* block, char* bitmap, int real, int fake) {
  	int difference = 0;
  	difference = abs(real - fack);
...
void fix_file_type(struct ext2_dir_entry *dir_entry) {
  	...
  	// if not match, fix it.
  	if (!compare_mode_type(dir_entry, inode)){
...
void fix_blocks_allocated(unsigned int index){
  	...
  	for (i = 0; i < inode->i_blocks / 2; i++){
    	...
    	// for indirect blocks
      		...
      		// break;
      		fixed_i = (unsigned int *)(disk + inode->i_block[12] * EXT2_BLOCK_SIZE)[i - 13];
    	}
